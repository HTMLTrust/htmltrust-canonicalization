...
class Canonicalize
{
    ...
    private static function preflightHtmlSource(string $html): void
    {
        ...
        // alternation; malformed input could make PCRE retry the same span
        // exponentially. This scanner advances monotonically and only keeps
        // the opening and matching closing tags.
        $scan = self::stripExcludedElementContents($html);
        ...
        $stack = [];
        $index = 0;
        foreach (self::iterProfileTokens($scan) as [$offset, $tokenEnd]) {
            $token = substr($scan, $offset, $tokenEnd - $offset);
    ...
    /**
     * Strip paired script/style/iframe bodies before source validation.
     *
     * This replaces the old nested body regex. A complete candidate opening
     * tag is scanned once for its first unquoted `>`, then its matching close
     * is located with a monotonic search. The retained source is equivalent
     * to replacing the body with an empty string, while malformed candidates
     * are rejected before DOMDocument can repair them.
     */
    private static function stripExcludedElementContents(string $source): string
    {
        $excluded = ['script', 'style', 'iframe'];
        $output = [];
        $cursor = 0;
        $length = strlen($source);

        while ($cursor < $length) {
            $start = self::findExcludedStart($source, $cursor, $excluded);
            if ($start === null) {
                $output[] = substr($source, $cursor);
                break;
            }

            $output[] = substr($source, $cursor, $start - $cursor);
            $openingEnd = self::scanTagEnd($source, $start);
            if ($openingEnd === null) {
                throw new \InvalidArgumentException('parser-profile-unsupported');
            }
            $name = self::startTagName($source, $start, $excluded);
            if ($name === null) {
                $output[] = substr($source, $start, $openingEnd - $start);
                $cursor = $openingEnd;
                continue;
            }
            $closing = self::findMatchingClose($source, $openingEnd, $name);
            if ($closing === null) {
                // Keep a complete but unpaired opening available to the
                // source-depth check, which reports the missing end tag.
                $output[] = substr($source, $start);
                break;
            }
            [$closeStart, $closeEnd] = $closing;
            $output[] = substr($source, $start, $openingEnd - $start);
            $output[] = substr($source, $closeStart, $closeEnd - $closeStart);
            $cursor = $closeEnd;
        }
    ...
    private static function findExcludedStart(string $source, int $offset, array $excluded): ?int
    {
        $index = $offset;
        while (($index = strpos($source, '<', $index)) !== false) {
            if (self::startTagName($source, $index, $excluded) !== null) {
                return $index;
            }
            $index++;
        }
    ...
    /**
     * Return an excluded start-tag name using the former `<\s*name\b`
     * spelling. The word-boundary check intentionally permits punctuation,
     * including qualified names, after the excluded name.
     *
     * @param array<int, string> $names
     */
    private static function startTagName(string $source, int $start, array $names): ?string
    {
        if (($source[$start] ?? '') !== '<') {
            return null;
        }
        $index = $start + 1;
        while ($index < strlen($source) && self::isHtmlSpace($source[$index])) {
            $index++;
        }
        if ($index >= strlen($source) || !self::isAsciiLetter($source[$index])) {
            return null;
        }
        foreach ($names as $name) {
            $nameLength = strlen($name);
            if (strncasecmp(substr($source, $index, $nameLength), $name, $nameLength) !== 0) {
                continue;
            }
            $after = $index + $nameLength;
            if ($after < strlen($source) && self::isAsciiWord($source[$after])) {
                continue;
            }
            return $name;
        }
    ...
    private static function findMatchingClose(string $source, int $offset, string $name): ?array
    {
        $index = $offset;
        $length = strlen($source);
        $nameLength = strlen($name);
        while (($index = strpos($source, '<', $index)) !== false) {
            ...
            $cursor++;
            while ($cursor < $length && self::isHtmlSpace($source[$cursor])) {
                $cursor++;
            }
            if (strncasecmp(substr($source, $cursor, $nameLength), $name, $nameLength) !== 0) {
                $index++;
                continue;
            }
            $cursor += $nameLength;
    ...
    private static function iterProfileTokens(string $source): array
    {
        $tokens = [];
        $cursor = 0;
        $length = strlen($source);
        while ($cursor < $length) {
            ...
            if (substr($source, $start, 4) === '<!--') {
                $commentEnd = strpos($source, '-->', $start + 4);
                if ($commentEnd === false) {
                    break;
                }
                $tokenEnd = $commentEnd + 3;
                $tokens[] = [$start, $tokenEnd];
                $cursor = $tokenEnd;
                continue;
            }
            if (!self::looksLikeProfileTag($source, $start)) {
                $cursor = $start + 1;
                continue;
            }
            $tokenEnd = self::scanTagEnd($source, $start);
            if ($tokenEnd === null) {
                throw new \InvalidArgumentException('parser-profile-unsupported');
            }
            $tokens[] = [$start, $tokenEnd];
    ...
    private static function looksLikeProfileTag(string $source, int $start): bool
    {
        $index = $start + 1;
        if (($source[$index] ?? '') === '/') {
            $index++;
        }
        return $index < strlen($source) && self::isAsciiLetter($source[$index]);
    }
    ...
    private static function isAsciiWord(string $char): bool
    {
        $ord = ord($char);
        return self::isAsciiLetter($char) || ($ord >= 0x30 && $ord <= 0x39) || $char === '_';
    }
